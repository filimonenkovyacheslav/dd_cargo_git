...
<div class="content mt-3">
	<div class="animated fadeIn">
		<div class="row">
			<div class="col-md-12">
				<a href="{{ route('exportExcelEngDraft') }}" style="margin-bottom: 20px;" class="btn btn-success btn-move">Export to Excel</a>
			</div>
		</div>
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					...
					<div class="btn-move-wrapper" style="display:flex">
						<form action="{{ route('engDraftWorksheetFilter') }}" method="GET" id="form-worksheet-table-filter" enctype="multipart/form-data">
							@csrf
						...
						<div class="table-container">
							...
							<table class="table table-striped table-bordered">
								...
								<tbody>
									...
									@if(isset($eng_draft_worksheet_obj))
									@foreach($eng_draft_worksheet_obj as $row)
									...
									<tr>
										...
										<td class="td-button">
											...
											<a class="btn btn-primary" href="{{ url('/admin/eng-draft-worksheet/'.$row->id) }}">Change</a>

											@endcan

											@can('activateEngDraft')

											<a class="btn btn-success" data-id="{{ $row->id }}" onclick="ConfirmActivate(event)" href="{{ url('/admin/eng-draft-check-activate/'.$row->id) }}">Activate</a>

											@endcan

											@can('editPost')

											{!! Form::open(['url'=>route('deleteEngDraftWorksheet'),'onsubmit' => 'return ConfirmDelete()', 'class'=>'form-horizontal','method' => 'POST']) !!}
							...
							@if(isset($data))
							{{ $eng_draft_worksheet_obj->appends($data)->links() }}
							@else
							{{ $eng_draft_worksheet_obj->links() }}
							@endif
							...
							<div class="checkbox-operations">
								...
								{!! Form::open(['url'=>route('addEngDraftDataById'), 'class'=>'worksheet-add-form','method' => 'POST']) !!}
								...
								{!! Form::open(['url'=>route('deleteEngDraftWorksheetById'),'onsubmit' => 'return ConfirmDelete()','method' => 'POST']) !!}
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.error) {
						...
						if (phone) location.href = '/admin/eng-draft-activate/'+rowId;						
					}
					else{
						location.href = '/admin/eng-draft-activate/'+rowId;
					}
				},
				error: function (msg) {
					alert('Ошибка admin');
